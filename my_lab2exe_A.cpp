...
int my_strlen(const char *s){
    int count = 0;
    // include a maximum length to prevent infinite loop
...
void my_strncat(char *dest, const char *source, int num){
    // find null character
    // used MAX_LENGTH to prevent infinite loop
    for(int i = 0; i < MAX_LENGTH; i++){
        if(dest[i] == 0){
            // append characters to end of dest
            for(int j = 0; j < num; j++){
                dest[i + j] = source[j];
            }
            // ensure new string is null terminated
            dest[i + num] = '\0';
            // stop loop
            break;
        }
